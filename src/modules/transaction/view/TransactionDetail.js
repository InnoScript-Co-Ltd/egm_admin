...
export const TransactionDetail = () => {
  ...
  const paymentTranscationHandler = async () => {
    ...
    const result = await transactionService.makePayment(
      dispatch,
      transcationId.current
    );

    if (result.status === 200) {
      const backward =
        transaction.sender_type === "MAIN_AGENT" ||
        transaction.sender_type === "SUB_AGENT"
          ? `${paths.transaction}/agent/DEPOSIT_PENDING`
          : `${paths.transaction}/partner/DEPOSIT_PENDING`;
      navigate(backward);
    }
    setLoading(false);
  };
  ...
  const paymentRejectHandler = async () => {
    setLoading(true);
    const result = await transactionService.makeReject(
      dispatch,
      transaction.current
    );
      ...
      <div className="flex flex-row justify-content-between align-items-center">
          ...
          <Badge
            ...
            severity={`${
              depositStatus.current === "DEPOSIT_PENDING"
                ? "warning"
                : depositStatus.current === "DEPOSIT_REJECT"
                ? "danger"
                : "success"
            }`}
          ></Badge>
        ...
        <div className="flex flex-row justify-content-end align-items-center">
          ...
          <Button
            ...
            onClick={() =>
              navigate(
                `${paths.transaction}/${depositStatus.current}/${transcationId.current}/edit`
              )
            }
          >
            Edit Transaction
  ...
  return (
    <div className="grid">
      ...
      <div className="col-12">
        {!loading && transaction && (
          <Card title={<CardTitleView />}>
            <div className="grid">
              <div className="col-12 md:col-4 lg:col-4 py-3">
                <h4 className="py-3" style={{ fontSize: "20px" }}>
                  {" "}
                  Agent Bank Account Information{" "}
                </h4>
              ...
              <div className="col-12 md:col-4 lg:col-4 py-3">
                <h3 className="py-3" style={{ fontSize: "20px" }}>
                  {" "}
                  Package Information{" "}
                </h3>
                ...
                <div className="w-full flex flex-row align-items-center justify-content-between py-2 mb-3">
                  ...
                  <small>
                    {" "}
                    {numeral(transaction.package_deposit_amount).format(
                      "0,0"
                    )}{" "}
                    Kyats{" "}
                  </small>
        ...
        >
          ...
          <div className="flex flex-row justify-content-end align-items-center mt-3">
            <Button
              ...
              size="small"
              severity="success"
              onClick={() => paymentTranscationHandler()}
            >
              {" "}
              Approve{" "}
            </Button>
          ...
          <Column
            ...
            body={(rowData) => (
              <span
                ...
                onClick={() =>
                  navigate(
                    `${paths.deposit}/agent/${transaction.id}/${paths.repayment}/${rowData.id}`
                  )
                }
              >
