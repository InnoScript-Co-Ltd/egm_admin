...
import { setPaginate } from "../bankAccountTypeSlice";
import { bankAccountTypePayload } from "../bankAccountTypePayload";
import { bankAccountTypeService } from "../bankAccountTypeService";

export const BankAccountTypeTableView = () => {

  const [loading, setLoading] = useState(false);
  const [showAuditColumn, setShowAuditColumn] = useState(false);

  const columns = useRef(bankAccountTypePayload.columns);
      ...
      <div className="flex items-center justify-content-between">
        ...
        <div className=" flex align-items-center gap-3">
          <Button
            ...
            size="small"
            onClick={() => {
              dispatch(setPaginate(bankAccountTypePayload.paginateParams));
  ...
  const HeaderRender = () => {
    return (
      <div className="w-full flex flex-column md:flex-row justify-content-between md:justify-content-start align-items-start md:align-items-center gap-3">
        <Search
          tooltipLabel={bankAccountTypePayload.searchableFields}
  ...
  // const tableData = useMemo(() => bankAccountTypes || [], [bankAccountTypes]);

  return (
    <Card title={"Bank Account Type List"}>
      <DataTable
        dataKey="id"
        size="normal"
        ...
        globalFilterFields={bankAccountTypePayload.columns}
        header={<HeaderRender />}
        footer={<FooterRender />}
      >
              ...
              body={(value) => {
                switch (col.field) {
                  case "bank_name":
                    return (
                      <NavigateId
                        url={`${paths.bankAccountType}/${value["id"]}`}
