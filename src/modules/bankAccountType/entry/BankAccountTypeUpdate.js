...
export const BankAccountTypeUpdate = () => {
  ...
  const bankAccountTypeUpdate = async () => {
    setLoading(true);
    const response = await bankAccountTypeService.update(
      dispatch,
      payload,
      params.id
    );
  ...
  const loadingData = useCallback(async () => {
    setLoading(true);
    const resultPartner = await getRequest(
      `${endpoints.status}?type=bankAccountType`
    );
          ...
          <div className="grid">
            <div className="col-12 md:col-12 lg:col-12 py-3 text-center">
              <img
                src={
                  preview ||
                  `https://api.evanglobalmanagement.com/storage/images/${payload.logo}`
                }
                style={{
                  width: "80px",
                  height: "80px",
