...
import { setPaginate } from "../usdtSlice";
import { usdtPayload } from "../usdtPayload";
import { usdtService } from "../usdtService";

export const USDTTableView = () => {
  const [loading, setLoading] = useState(false);
  const [showAuditColumn, setShowAuditColumn] = useState(false);

  const columns = useRef(usdtPayload.columns);
  ...
  const { usdts, paginateParams } = useSelector((state) => state.usdt);
  ...
  const loadingData = useCallback(async () => {
    setLoading(true);
    const result = await usdtService.index(dispatch, paginateParams);
  ...
  useEffect(() => {
    loadingData();
  }, [loadingData, paginateParams]); // Ensure re-fetch when params change
      ...
      <div className="flex items-center justify-content-between">
        ...
        <div className=" flex align-items-center gap-3">
          <Button
            ...
            size="small"
            onClick={() => {
              dispatch(setPaginate(usdtPayload.paginateParams));
  ...
  const HeaderRender = () => {
    return (
      <div className="w-full flex flex-column md:flex-row justify-content-between md:justify-content-start align-items-start md:align-items-center gap-3">
        <Search
          tooltipLabel={usdtPayload.searchableFields}
  ...
  // const tableData = useMemo(() => usdts || [], [usdts]);

  return (
    <Card title={"USDT List"}>
      <DataTable
        dataKey="id"
        size="normal"
        value={usdts}
        ...
        emptyMessage="No bank account types found."
        globalFilterFields={usdtPayload.columns}
        footer={<FooterRender />}
      >
        {showColumns.current.map((col, index) => {
          return (
            <Column
              key={`bank_account_types_col_index_${index}`}
