...
import { setPaginate } from "../bonusPointSlice";
import { bonusPointPayload } from "../bonusPointPayload";
import { bonusPointService } from "../bonusPointService";
import numeral from "numeral";

export const BonusPointTableView = () => {
    const { bonusPoints, paginateParams } = useSelector(state => state.bonusPoint);

    const [loading, setLoading] = useState(false);
    const [showAuditColumn, setShowAuditColumn] = useState(false);

    const columns = useRef(bonusPointPayload.columns);
 ...
 */
    ...
    const loadingData = useCallback(async () => {
        ...
        const response = await bonusPointService.index(dispatch, paginateParams);
            ...
            <div className=' flex items-center justify-content-between'>
                ...
                <div className=' flex align-items-center gap-3'>
                    <Button
                        ...
                        size="small"
                        onClick={() => {
                            dispatch(setPaginate(bonusPointPayload.paginateParams));
    ...
    const HeaderRender = () => {
        return (
            <div className="w-full flex flex-column md:flex-row justify-content-between md:justify-content-start align-items-start md:align-items-center gap-3">
                <Search
                    tooltipLabel={"Search Bonus Point"}
                    placeholder={"Search Bonus Point"}
    ...
    return (
        ...
        >
            <DataTable
                ...
                onSort={onSort}
                loading={loading}
                emptyMessage="No bonus points found."
                globalFilterFields={bonusPointPayload.columns}
            ...
            >
                {showColumns.current.map((col, index) => {
                    return (
                        <Column
                            key={`bonus_point_col_index_${index}`}
                                    ...
                                    case "action":
                                        return (
                                            <i
                                                ...
                                                onClick={() => navigate(`/${paths.bonusPoint}/${value.id}`)}
                                            ></i>
                                        );
                                    case "limit_amount":
                                        return (
                                            <span> {numeral(value[col.field]).format('0,0')} (EMU)</span>
                                        );
                                    case "status":
